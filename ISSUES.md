...
- **실패 메커니즘**: Kalman filter forward/backward pass에서 수치적 오차 누적 → smoother에서 NaN/Inf 전파 → 손상된 EZZ로 인한 sum_EZZ ill-conditioning → C matrix NaN
- **근본 원인**: F matrix inversion 자체는 문제가 아님 (condition number 정상). 실제 문제는 Kalman filter의 forward/backward pass에서 수치적 오차가 누적되어 NaN/Inf가 전파되는 것
...
### 2. Kalman Filter의 수치적 오차 누적 및 NaN/Inf 전파

코드 위치: `dfm-python/src/dfm_python/ssm/kalman.py:350-600`

**수학적 관계**:
- `F = C_t @ V @ C_t.T + R_t`: (n_obs, n_obs) - innovation covariance
- `V`: (15, 15) - factor covariance (prior)
- `Vu`: (15, 15) - posterior covariance
- `C_t`: (n_obs, 15) - t 시점에서 관측된 시리즈들의 loading matrix
- `R_t`: (n_obs, n_obs) - t 시점에서 관측된 시리즈들의 measurement error covariance

**핵심 발견 (실험 4에서 확인)**:
- **F matrix inversion 자체는 문제가 아님**: F matrix condition number가 정상 (처음 20개 시간 단계에서 모두 1e8 미만)
- **실제 문제**: Kalman filter의 forward/backward pass에서 수치적 오차가 누적되어 NaN/Inf가 전파됨

**문제 메커니즘**:
1. **Forward Pass에서 수치적 오차 누적**:
   - F matrix inversion은 성공하지만, V, Vu 업데이트 과정에서 수치적 오차 누적
   - `Vu = V - VCF @ VC.T` (posterior covariance 업데이트)
   - `V = A @ Vu @ A.T + Q` (다음 시간의 prior covariance)
   - 특히 n_obs=33일 때 작은 수치 오차도 증폭
   - 여러 시간 단계를 거치면서 오차가 누적되어 NaN/Inf 발생

2. **Backward Pass (Smoother)에서 NaN/Inf 전파**:
   - Forward pass에서 손상된 V, Vu가 backward pass로 전파
   - `VmT` recursion에서 `VmU` (forward pass 결과)가 이미 NaN/Inf 포함
   - Backward pass는 forward pass 결과에 의존하므로 손상 전파
   - Smoothed factors (`EZ`, `EZZ`)가 손상됨
   - 로그에서 확인: `smoother_backward: VmT[:, :, t] recursion produced NaN/Inf`

**로그에서 확인된 사실** (검증됨):
- **KOEQUIPTE**: 
  - 시리즈 개수: 32개 (로그: "Series: 32 (filtered from config)")
  - NaN/Inf 발생 없음
  - F matrix condition number 정상
- **KOIPALL.G**: 
  - 시리즈 개수: 33개 (로그: "Series: 33 (filtered from config)")
  - F matrix condition number: 처음 20개 시간 단계 모두 정상 (1e8 미만) ✅
  - 하지만 smoother에서 NaN/Inf 전파 발생 (로그: "smoother_backward: VmT[:, :, t] recursion produced NaN/Inf")
  - 손상된 EZZ로 인한 sum_EZZ eigendecomposition 실패
- **KOWRCCNSE**: 
  - 시리즈 개수: 39개 (로그: "Series: 39 (filtered from config)")
  - F matrix condition number 정상
  - 하지만 smoother에서 NaN/Inf 전파 발생

**관찰된 패턴** (검증됨):
- KOEQUIPTE (32개 시리즈): 안정적 ✅
- KOIPALL.G (33개 시리즈): F matrix는 정상이지만 smoother에서 NaN/Inf 전파 ❌
- KOWRCCNSE (39개 시리즈): F matrix는 정상이지만 smoother에서 NaN/Inf 전파 ❌

**핵심 통찰**:
- F matrix inversion 자체는 문제가 아님 (condition number 정상)
- 실제 문제는 forward/backward pass에서 수치적 오차 누적
- n_obs가 클수록 (≥33) 수치적 오차가 더 빠르게 누적되어 NaN/Inf 발생
...
**문제 메커니즘**:
1. **Kalman filter smoother에서 NaN/Inf 전파로 인한 EZZ 손상**:
   - Forward pass에서 수치적 오차 누적으로 인해 V, Vu가 손상됨
   - Backward pass (smoother)에서 손상된 VmU로 인해 NaN/Inf 전파
   - 손상된 Kalman filter smoother로 인해 smoothed factors (`EZ`, `EZZ`)가 손상됨
...
**전체 실패 경로** (수정됨 - 실험 4에서 확인):

1. **Kalman Filter Forward Pass** (코드: `dfm-python/src/dfm_python/ssm/kalman.py:350-450`)
   - `F = C_t @ V @ C_t.T + R_t` 계산
   - `F` matrix inversion은 성공 (condition number 정상) ✅
   - 하지만 V, Vu 업데이트 과정에서 수치적 오차 누적
   - `Vu = V - VCF @ VC.T` (posterior covariance 업데이트) 시 작은 오차 누적
   - 다음 시간의 `V = A @ Vu @ A.T + Q` (prior covariance) 계산 시 오차 누적
   - 여러 시간 단계를 거치면서 오차가 누적되어 NaN/Inf 발생
...
1. C Matrix NaN 처리 개선 (이전 값 유지) ⭐⭐ **✅ 구현 완료**
2. Kalman filter forward pass NaN/Inf 전파 근본 원인 해결 **🔄 진행 중**
3. 더 강한 regularization (1e-3보다 더 큰 값, 예: 1e-2) 고려

### 실험 4: C Matrix NaN 처리 개선 + 근본 원인 탐색 (2025-12-07 21:38)

#### 구현 완료 ✅
- **C Matrix NaN 처리 개선**: `em.py:311-324`
  - NaN 발생 시 이전 iteration의 값 유지 (0으로 설정하는 대신)
  - 점진적 개선 가능
  - EM algorithm의 점진적 수렴 특성 활용
- **F Matrix 진단 로깅 추가**: `kalman.py:393-409`
  - F matrix condition number 계산 및 로깅
  - F matrix inversion 전후 상태 확인
  - n_obs, adaptive_reg, missing_ratio 정보 포함

#### 실험 결과
- **KOIPALL.G**: 
  - F matrix condition number: 처음 20개 시간 단계 모두 정상 (1e8 미만) ✅
  - C matrix NaN: 97.0% → 100.0% (여전히 높음) ❌
  - C matrix NaN 값 보존: 4704개 (이전 iteration에서) ✅
  - sum_EZZ eigendecomposition 실패: 계속 발생 (error code 49) ❌

#### 근본 원인 발견 🔍

**핵심 발견**: F matrix inversion은 문제가 아님!
- F matrix condition number가 정상 (처음 20개 시간 단계에서 모두 1e8 미만)
- F matrix inversion 자체는 성공적으로 수행됨

**실제 근본 원인**: Kalman filter smoother에서 NaN/Inf 전파
1. **Kalman filter smoother (backward pass)에서 NaN/Inf 발생**:
   - `VmT[:, :, t] recursion produced NaN/Inf` (로그에서 확인)
   - Smoothed factors (`EZ`, `EZZ`)가 손상됨
2. **손상된 EZZ로 인한 sum_EZZ ill-conditioning**:
   - `sum_EZZ = sum_t EZZ` 계산 시 손상된 EZZ 포함
   - sum_EZZ eigendecomposition 실패 (error code 49)
3. **C matrix 업데이트 실패**:
   - sum_EZZ eigendecomposition 실패 → condition number 계산 불가
   - 1e-3 regularization도 충분하지 않음
   - C matrix가 NaN으로 채워짐 (97-100%)

**왜 smoother에서 NaN/Inf가 발생하는가?**
- Forward pass에서 손상된 V, Vu가 backward pass로 전파
- `VmT` recursion에서 `VmU` (forward pass 결과)가 이미 NaN/Inf 포함
- Backward pass는 forward pass 결과에 의존하므로 손상 전파

**왜 forward pass에서 V, Vu가 손상되는가?**
- F matrix inversion은 성공하지만, V, Vu 업데이트 과정에서 수치적 오차 누적
- 특히 n_obs=33일 때 작은 수치 오차도 증폭
- 여러 시간 단계를 거치면서 오차가 누적되어 NaN/Inf 발생

#### 결론
F matrix inversion 자체는 문제가 아니며, Kalman filter의 forward/backward pass에서 수치적 오차가 누적되어 NaN/Inf가 전파되는 것이 근본 원인입니다.

**가장 적절한 해결책** (우선순위별):
1. **EZZ의 NaN/Inf 제거** (우선순위 1) ⭐⭐⭐ **✅ 구현 완료**
   - sum_EZZ 계산 전에 EZZ의 NaN/Inf 제거
   - 손상된 EZZ를 0으로 대체 (해당 시간 단계 제외)
   - 이렇게 하면 sum_EZZ eigendecomposition 성공 가능
   - 구현 위치: `em.py:247-260`

2. **Backward pass에서 NaN/Inf 전파 방지** (우선순위 2) ⭐⭐ **✅ 이미 구현됨**
   - VmU가 NaN/Inf면 fallback 사용
   - 구현 위치: `kalman.py:577-623`

3. **Forward pass에서 V, Vu 안정화 강화** (우선순위 3) ⭐ **✅ 이미 구현됨**
   - 이미 많은 안전장치 추가됨
   - 구현 위치: `kalman.py:440-506`

**MATLAB vs Python 차이점**:
- **MATLAB 구현**: 더 단순함 (NaN/Inf 체크 없음), NaN/Inf 발생 시 그냥 실패하거나 무시
- **Python 구현 (우리)**: 더 엄격함 (NaN/Inf 체크 및 처리), 하지만 근본 원인 해결 필요

**왜 다른 연구자는 안정적이라고 주장하는가?**
1. 다른 데이터셋/설정 사용 (시리즈 개수 ≤32, missing data 적음, state space 차원 다름)
2. MATLAB의 내부 처리 (inv()가 내부적으로 더 robust하게 처리 가능)
3. 문제를 숨기고 있을 수 있음 (NaN/Inf 발생해도 결과가 나오면 '성공'으로 간주)

**결론**: MATLAB이 '사기치는' 것은 아님. 우리 데이터/설정이 더 어려움 (33개 이상 시리즈)이거나, MATLAB도 같은 문제가 있지만 무시하고 있을 수 있음.
...
## VAR-1 Persistence 문제 (2025-12-07)

### 문제 요약
VAR 모델의 horizon 1 예측이 비정상적으로 좋은 결과를 보임 (sMSE < 1e-6). 이는 VAR이 실제 예측을 하지 않고 마지막 training value를 그대로 예측하는 persistence 문제로 확인됨.

### 증상
- **VAR-1 sMSE**: 3.5e-09 ~ 5.8e-09 (비정상적으로 작음)
- **VAR-1 sMAE**: 5.9e-05 ~ 7.6e-05 (비정상적으로 작음)
- **VAR-7, VAR-28**: 극단적으로 큰 값 (수치적 불안정성)

### 원인
1. **VAR-1 persistence**: VAR 모델이 horizon 1에서 마지막 training value를 그대로 예측
2. **Persistence detection 미작동**: `evaluate_forecaster()`의 persistence detection 로직이 제대로 작동하지 않음
3. **CSV에 작은 값 저장**: 테이블 생성 시점에만 NaN 처리되고, CSV에는 원래 값이 저장됨

### 해결 방안
✅ **구현 완료** (2025-12-07):
- `generate_all_latex_tables()`에서 VAR-1 persistence detection 강화
- sMSE < 1e-6 또는 sMAE < 1e-4인 VAR-1 행을 모두 NaN으로 처리
- 로직 개선: `persistence_detected` 플래그 대신 직접 mask 계산

### 다음 단계
1. 테이블 재생성: `python3 -c "from src.evaluation import generate_all_latex_tables; generate_all_latex_tables()"`
2. 보고서 업데이트: VAR-1 결과가 N/A로 표시되는지 확인
