#!/bin/bash
# DDFM training script
# 
# This script trains DDFM models for specified targets.
scripts/run_train_ddfm.sh# Models are saved to checkpoints/{target}_ddfm/model.pkl
#
# Usage:
#   bash scripts/run_train_ddfm.sh                    # Train all targets
#   bash scripts/run_train_ddfm.sh --target KOEQUIPTE # Train specific target
#   bash scripts/run_train_ddfm.sh --test             # Test mode (reduced epochs)
...
cleanup() {
    if [ $SHUTDOWN_REQUESTED -eq 0 ]; then
        ...
        PYTHON_PIDS=$(pgrep -f "train_common.*ddfm" 2>/dev/null || true)
        ...
        REMAINING=$(pgrep -f "train_common.*ddfm" 2>/dev/null || true)
        if [ -n "$REMAINING" ]; then
...
if [ $TEST_MODE -eq 1 ]; then
    ...
    echo "=========================================="
    echo "TEST MODE: Running with reduced epochs"
    echo "=========================================="
else
    CHECKPOINT_BASE="checkpoints"
    echo "=========================================="
    echo "Starting DDFM training"
    echo "=========================================="
fi
...
if [ $TEST_MODE -eq 1 ]; then
    echo "Training: Reduced epochs (TEST MODE)"
else
    echo "Training: Normal epochs"
fi
echo "=========================================="
echo ""
...
" 2>/dev/null || true
    ...
    LOG_FILE="$PROJECT_ROOT/log/${TARGET}_ddfm_${TIMESTAMP}.log"
    
    # Build Hydra override arguments
    HYDRA_OVERRIDES=(
        "--config-name" "$CONFIG"
        "experiment.model=ddfm"
    ...
    # Add test mode overrides (reduced epochs for DDFM)
    if [ $TEST_MODE -eq 1 ]; then
        HYDRA_OVERRIDES+=("experiment.model_overrides.ddfm.epochs=5")
        HYDRA_OVERRIDES+=("experiment.model_overrides.ddfm.max_iter=5")
    fi
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        echo "[$TARGET] ✓ DDFM completed successfully"
        echo "  Checkpoint: $MODEL_FILE"
    else
        if [ $SHUTDOWN_REQUESTED -eq 1 ]; then
            echo "[$TARGET] ⚠ DDFM interrupted by user"
            break
        else
            echo "[$TARGET] ✗ DDFM failed (exit code: $EXIT_CODE)"
...
if [ ${#FAILED[@]} -eq 0 ]; then
    if [ $SHUTDOWN_REQUESTED -eq 1 ]; then
        ...
        echo "✓ All DDFM models trained successfully!"
        echo ""
        echo "Checkpoints saved to: $CHECKPOINT_BASE/"
        echo "  - ${TARGETS_TO_TRAIN[@]/%/_ddfm/model.pkl}"
        exit 0
    fi
else
    echo "✗ Some DDFM models failed:"
