#!/bin/bash
# Chronos training script
# 
# This script trains Chronos models for specified targets.
# Chronos is a pre-trained foundation model, so "training" involves fitting
# the model to the data (no actual training epochs needed).
# Models are saved to checkpoints/ directory.
#
# Usage:
#   bash scripts/run_train_chronos.sh                    # Train all targets
#   bash scripts/run_train_chronos.sh --target KOEQUIPTE  # Train specific target
#   bash scripts/run_train_chronos.sh --test             # Test mode
...
if [ $TEST_MODE -eq 1 ]; then
    ...
    echo "=========================================="
    echo "TEST MODE: Running test training"
    echo "=========================================="
else
    CHECKPOINT_BASE="checkpoints"
    echo "=========================================="
    echo "Chronos Training"
    echo "=========================================="
fi
...
for TARGET in "${TARGETS_TO_PROCESS[@]}"; do
    ...
    echo "----------------------------------------"
    echo "[$TARGET] Training Chronos model"
    echo "Config: $CONFIG"
    echo "----------------------------------------"
    ...
    if [ $TEST_MODE -eq 0 ]; then
        if [ -f "$MODEL_FILE" ]; then
            echo "[$TARGET] ⏭ Chronos checkpoint already exists at $MODEL_FILE, skipping..."
            echo ""
            continue
        fi
    fi
...
" 2>/dev/null || true
    ...
    if python -m src.train.train_common "${HYDRA_OVERRIDES[@]}" 2>&1 | tee "$LOG_FILE"; then
        if [ -f "$MODEL_FILE" ]; then
            echo "[$TARGET] ✓ Chronos training completed"
            ...
            if python -c "
from pathlib import Path
from src.models.base import load_model_checkpoint
try:
    model, metadata = load_model_checkpoint(Path('$MODEL_FILE'))
    print('  ✓ Checkpoint verification: Loaded successfully')
    print(f'    Model type: {metadata.get(\"model_type\", \"unknown\")}')
    print(f'    Target: {metadata.get(\"target_series\", \"unknown\")}')
...
" 2>&1; then
                ...
                echo "  ⚠ Checkpoint verification failed (but file exists)"
            fi
        else
            echo "[$TARGET] ✗ Chronos training completed but checkpoint not found"
            echo "  Log: $LOG_FILE"
            FAILED+=("${TARGET}_checkpoint_missing")
        fi
    else
        echo "[$TARGET] ✗ Chronos training failed"
        echo "  Log: $LOG_FILE"
...
# Summary
echo "=========================================="
echo "Chronos Training Complete"
echo "=========================================="

if [ ${#FAILED[@]} -eq 0 ]; then
    echo "✓ All Chronos models trained successfully!"
    echo ""
    echo "Generated checkpoints:"
    for TARGET in "${TARGETS_TO_PROCESS[@]}"; do
        CHECKPOINT_DIR="$PROJECT_ROOT/$CHECKPOINT_BASE/${TARGET}_${MODEL}"
        if [ -f "$CHECKPOINT_DIR/model.pkl" ]; then
    ...
    echo "  2. Run forecasts: bash scripts/run_forecast_chronos.sh"
