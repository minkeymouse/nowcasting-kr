...
# 
# This script trains TFT models for specified targets.
# Models are saved to checkpoints/ directory.
#
# Usage:
#   bash scripts/run_train_tft.sh                    # Train all targets
#   bash scripts/run_train_tft.sh --target KOEQUIPTE  # Train specific target
#   bash scripts/run_train_tft.sh --test             # Test mode (1 epoch)
...
for TARGET in "${TARGETS_TO_PROCESS[@]}"; do
    ...
    echo "----------------------------------------"
    echo "[$TARGET] Training TFT model"
    echo "Config: $CONFIG"
    echo "----------------------------------------"
    ...
    MODEL_FILE_ZIP="${CHECKPOINT_DIR}/model.zip"
    
    if [ $TEST_MODE -eq 0 ]; then
        if [ -f "$MODEL_FILE" ] || [ -f "$MODEL_FILE_ZIP" ]; then
            if [ -f "$MODEL_FILE" ]; then
                echo "[$TARGET] ⏭ TFT checkpoint already exists at $MODEL_FILE, skipping..."
            else
                echo "[$TARGET] ⏭ TFT checkpoint already exists at $MODEL_FILE_ZIP, skipping..."
            fi
            echo ""
            continue
        fi
...
" 2>/dev/null || true
    ...
    # Add test mode overrides (1 epoch for TFT)
    if [ $TEST_MODE -eq 1 ]; then
        HYDRA_OVERRIDES+=("experiment.model_overrides.tft.max_epochs=1")
    ...
    if python -m src.train.train_common "${HYDRA_OVERRIDES[@]}" 2>&1 | tee "$LOG_FILE"; then
        if [ -f "$MODEL_FILE" ] || [ -f "$MODEL_FILE_ZIP" ]; then
            echo "[$TARGET] ✓ TFT training completed"
            if [ -f "$MODEL_FILE" ]; then
                echo "  Checkpoint: $MODEL_FILE"
            else
                echo "  Checkpoint: $MODEL_FILE_ZIP"
            fi
            echo "  Log: $LOG_FILE"
        else
            echo "[$TARGET] ✗ TFT training completed but checkpoint not found"
            echo "  Log: $LOG_FILE"
            FAILED+=("${TARGET}_checkpoint_missing")
        fi
    else
        echo "[$TARGET] ✗ TFT training failed"
        echo "  Log: $LOG_FILE"
...
if [ ${#FAILED[@]} -eq 0 ]; then
    echo "✓ All TFT models trained successfully!"
    echo ""
    ...
    for TARGET in "${TARGETS_TO_PROCESS[@]}"; do
        ...
        if [ -f "$CHECKPOINT_DIR/model.pkl" ]; then
            ...
            echo "  - $CHECKPOINT_BASE/${TARGET}_${MODEL}/model.zip"
        fi
    done
    echo ""
    echo "Next steps:"
    echo "  1. Review training logs in log/"
    echo "  2. Run forecasts: bash scripts/run_forecast_tft.sh"
