#!/bin/bash
# LSTM forecasting script
# 
# This script generates forecasts from trained LSTM models for specified targets.
# Forecasts are saved to predictions/lstm/ directory.
#
# Usage:
#   bash scripts/run_forecast_lstm.sh                    # Forecast all targets (88 weeks)
#   bash scripts/run_forecast_lstm.sh --target KOEQUIPTE # Forecast specific target
#
# Prerequisites:
# - LSTM models must be trained first (run run_train_lstm.sh)
# - Checkpoints must exist in checkpoints/{target}_lstm/model.pkl
...
mkdir -p "$PROJECT_ROOT/predictions/lstm"
...
for TARGET in "${TARGETS_TO_PROCESS[@]}"; do
    ...
    echo "[$TARGET] Processing LSTM forecasts..."
    
    # Check if checkpoint exists
    if [ ! -f "$CHECKPOINT_FILE" ]; then
        echo "[$TARGET] ⚠ Checkpoint not found: $CHECKPOINT_FILE"
        echo "  Run 'bash scripts/run_train_lstm.sh --target $TARGET' to train the model first."
...
predictions_dir = project_root / 'predictions' / 'lstm'

logger.info('='*70)
logger.info(f'LSTM Forecasting for {target}')
logger.info('='*70)
...
try:
    ...
    weekly_file = predictions_dir / f'{target}_lstm_weekly.csv'
    ...
    monthly_file = predictions_dir / f'{target}_lstm_monthly.csv'
    monthly_forecast.to_csv(monthly_file, header=True)
    logger.info(f'Saved monthly forecast to: {monthly_file}')
    
    logger.info('='*70)
    logger.info(f'✓ LSTM forecasting completed for {target}')
    logger.info('='*70)
...
" 2>&1 | tee "$LOG_FILE"; then
        echo "[$TARGET] ✓ LSTM forecasting completed"
        echo "  Output: predictions/lstm/${TARGET}_lstm_weekly.csv"
    else
        echo "[$TARGET] ✗ LSTM forecasting failed"
...
# Summary
echo "=========================================="
echo "LSTM Forecasting Complete"
echo "=========================================="
...
if [ ${#FAILED[@]} -eq 0 ]; then
    ...
    for TARGET in "${TARGETS_TO_PROCESS[@]}"; do
        if [ -f "$PROJECT_ROOT/predictions/lstm/${TARGET}_lstm_weekly.csv" ]; then
            echo "  - predictions/lstm/${TARGET}_lstm_weekly.csv"
        fi
    done
    echo ""
    echo "Next steps:"
    echo "  1. Review forecasts in predictions/lstm/"
