"""Generate all figures for the report based on RESULTS_NEEDED.md.

This script generates placeholder figures when actual results are not available,
and real figures when results exist in outputs/.
"""
...
def generate_model_comparison_barplot(df: pd.DataFrame, output_path: Path):
    """Generate model comparison bar plot (fig:model_comparison)."""
...
def generate_horizon_trend_plot(df: pd.DataFrame, output_path: Path):
    """Generate horizon trend plot (fig:horizon_trend)."""
...
def generate_accuracy_heatmap(df: pd.DataFrame, output_path: Path):
    """Generate accuracy heatmap (fig:accuracy_heatmap)."""
...
def generate_forecast_vs_actual(df: pd.DataFrame, output_path: Path, target: str = 'GDP'):
    """Generate forecast vs actual time series plot."""
...
def generate_all_images(outputs_dir: Optional[Path] = None, 
                       images_dir: Optional[Path] = None):
    """Generate all report images according to RESULTS_NEEDED.md."""
    if outputs_dir is None:
        outputs_dir = Path(__file__).parent.parent.parent / "outputs"
    if images_dir is None:
        images_dir = Path(__file__).parent.parent.parent / "nowcasting-report" / "images"
    ...
    (images_dir / "model_comparison").mkdir(exist_ok=True)
    (images_dir / "horizon_analysis").mkdir(exist_ok=True)
    (images_dir / "heatmaps").mkdir(exist_ok=True)
    (images_dir / "forecasts").mkdir(exist_ok=True)
    ...
    generate_model_comparison_barplot(
        df, images_dir / "model_comparison" / "model_comparison.png"
    )
    
    # 6.2 Horizon trend
    print("\n  6.2 Horizon analysis plots...")
    generate_horizon_trend_plot(
        df, images_dir / "horizon_analysis" / "horizon_trend.png"
    )
    
    # 6.3 Heatmap
    print("\n  6.3 Heatmap plots...")
    generate_accuracy_heatmap(
        df, images_dir / "heatmaps" / "accuracy_heatmap.png"
    )
    
    # 6.4 Forecast vs actual
    print("\n  6.4 Forecast vs actual plots...")
    generate_forecast_vs_actual(
        df, images_dir / "forecasts" / "forecast_vs_actual.png", target='GDP'
    )
