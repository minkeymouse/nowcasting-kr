...
def evaluate_forecaster(
    ...
    try:
        if is_dfm_model:
            ...
            # For weekly models: predict 24 months (96 weeks) and convert to monthly by averaging every 4 weeks
            if clock == 'w':
                # Predict all weeks at once (model already updated with 2023-12 data in train.py)
                ...
                # Convert weekly forecasts to monthly by averaging every 4 weeks
                # X_forecast shape: (total_weeks, n_series)
                n_weeks, n_series = X_forecast.shape
                n_months = n_weeks // weeks_per_month
                
                # Reshape to (n_months, weeks_per_month, n_series) and average
                X_forecast_monthly = X_forecast[:n_months * weeks_per_month].reshape(
                    n_months, weeks_per_month, n_series
                ).mean(axis=1)  # Average over weeks -> (n_months, n_series)
                ...
                logger.info(f"Converted {n_weeks} weekly forecasts to {n_months} monthly forecasts (averaging every {weeks_per_month} weeks)")
            else:
            ...
            # For ARIMA/VAR: Generate recursive forecast once for all horizons
            # These models are not state-based, so we only need the last few periods
            # before the forecast start point. Recursive forecasting is handled by sktime.
            # Generate forecast for maximum horizon (recursive)
