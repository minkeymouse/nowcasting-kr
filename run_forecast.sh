#!/bin/bash
# Forecasting script - runs 22 horizon forecasts for all models
# Loads models from outputs/checkpoint/ directory and generates forecasts
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --models MODEL_LIST     Filter models (e.g., 'arima var dfm ddfm')"
            echo "  --jobs, -j N            Maximum parallel jobs (default: number of CPU cores)"
            echo "  --help, -h              Show this help message"
            echo ""
            echo "This script runs 22 horizon forecast (2024-01 to 2025-10, monthly)"
            echo "for all models: ARIMA, VAR, DFM, DDFM"
            echo ""
            echo "Results are saved to:"
            echo "  - outputs/comparisons/{target}/comparison_results.json"
            echo "  - outputs/comparisons/{target}/comparison_table.csv"
...
run_test_mode() {
    ...
    if [ ! -d "outputs/checkpoint" ]; then
        echo "  ✗ FAIL: outputs/checkpoint/ directory not found"
        test_failed=$((test_failed + 1))
    else
        echo "  ✓ PASS: outputs/checkpoint/ directory exists"
        test_passed=$((test_passed + 1))
    ...
    local dirs=("outputs/forecast" "outputs/comparisons" "outputs/experiments")
    for dir in "${dirs[@]}"; do
        if [ ! -d "$dir" ]; then
...
validate_checkpoints() {
    ...
    if [ $missing_count -gt 0 ]; then
        ...
        echo ""
        echo "Forecasting will skip missing models."
        if [ -t 0 ]; then
...
# ============================================================================
# 22 Horizon Forecast (2024-01 to 2025-10)
# ============================================================================

echo "=========================================="
echo "22 Horizon Forecast"
echo "=========================================="
...
run_forecast_target() {
    ...
    mkdir -p outputs/forecast || {
        echo "✗ Error: Cannot create outputs/forecast/ directory"
        return 1
    }
    local log_file="log/${target}_forecast.log"
    local CMD_ARGS=(
