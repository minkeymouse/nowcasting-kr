...
def validate_series_ids_for_config(
    series_ids: List[str],
    config_series_ids: List[str]
) -> Dict[str, Any]:
    """
    Validate series IDs against a model configuration.
    
    Parameters
    ----------
    series_ids : List[str]
        Series IDs to validate
    config_series_ids : List[str]
        Series IDs from model configuration
    ...
    Dict[str, Any]
        ...
        - valid: bool
        - missing: List[str] - series in config but not in input
        - extra: List[str] - series in input but not in config
        - matches: int - number of matching series
    """
    series_set = set(series_ids)
    config_set = set(config_series_ids)
    
    missing = list(config_set - series_set)
    extra = list(series_set - config_set)
    matches = len(series_set & config_set)
    ...
    return {
        ...
        'extra': extra,
        'matches': matches,
        'total_config': len(config_series_ids),
        'total_input': len(series_ids)
    }
